...
void insertAt(IntDoublyLL &ll, int index, int data)
{
    ...
    current->next->previous = newNode;  //kmyt lf w doran :) LOL
...
void removeFront(IntDoublyLL &ll)
{
    if (!isEmpty(ll))
    {
        ...
        if (ll.head->next != nullptr) // we should have used duscard here not ll.head 
...
void removeBack(IntDoublyLL &ll)
{
    ...
    if (!isEmpty(ll))
    {
        ...
        delete current->next; //supposed to be delete current
    }
    else
    {
        std::cout << "Empty list!" << std::endl;
